"""
    @description:       File for finding the best performing XGBClassifier for predicting segment mode
...
#RES_MERGED_COLS, TARGET_SEGNUMBER, FILTER_COLS, FILTER_COLS_MACARONI, FILTER_COLS_VEL, \
#    FILTER_COLS_ALLQ_BUT_MAC
from xgboost import XGBClassifier
from csv import DictWriter

def run_xgb_nseg(PARAMETERS):
    # ==================================================================
    # Specify paths
    # ==================================================================
    francis = True
    ...
    df_train = ld.load_rf_trainer_data(train_data_dir, verbose=True, slice=0.25)
    print("The data contains ", len(df_train), " samples.")
    print("INF values in df_train: ", df_train.index[np.isinf(df_train).any(1)])
    df_train = df_train.astype('float32').copy()
    df_train = df_train[~df_train.isin([np.nan, np.inf, -np.inf]).any(1)].copy()
    print("INF values in df_train: ", df_train.index[np.isinf(df_train).any(1)])
    print("The data contains ", len(df_train), " samples AFTER removing inf vals")
    print("Unique values in the dataset: ", df_train['n-HorznBitRates'].value_counts())
    # NOTE: modify accordin to the wize analysis plot
    # df_train['n-HorznBitRates'][df_train['n-HorznBitRates'] > 3] = 3.0
    df_train.loc[df_train['n-HorznBitRates'] > 3.0, 'n-HorznBitRates'] = 3.0
    print("n-HorznBitRates bigger than 3.0 removed")
    # ==================================================================
    ...
    #n_estimators = np.array([100, 200, 350])
    ...
    # Exploration grid
    xgb_grid = [(i, j) for i in n_estimators.tolist() for j in max_depth.tolist()]
    ...
    # NOTE: Select half of the data:
    # df_train = df_train[0: round(len(df_train)/4)]
    # print("The data now contains ", len(df_train), " samples.")
    ...
    xgb_winner["nseg_featcols"] = FEAT_COLS
    xgb_winner["nseg_targetcol"] = TARGET_SEGNUMBER
    ...
    for (estim, depth) in xgb_grid:
        ...
        classifier = XGBClassifier(n_estimators=estim, max_depth=depth,
        ...
        for train, test in cv.split(Xnseg_train, Ynseg_train):
            ...
            classifier.fit(Xnseg_train[train], Ynseg_train[train],
                           # eval_set=[(Xnseg_train[test], Ynseg_train[test])],
        ...
        print("### nseg mean ROC AUC in whole TRAIN CV set: ", mean_auc_TRAIN)
        ...
        for i in range(n_classes):
            ...
            # tmp_nan_indx = np.argwhere(np.isnan(tmp_y_test_score))
            # if tmp_nan_indx.size > 0:
            #     print("NAN VALUES INDEX: ", tmp_nan_indx)
                # print("NAN VALUES: ", tmp_y_test_score[tmp_nan_indx])
        ...
        if acc_test > max_acc_test:
            ...
            print("### AUC TEST: ", mean_auc_fold_test)
            print("### nseg Number of trees: ", estim)
            print("### Max depth of trees: ", depth)

            xgb_winner["logger_index"] = logger_index
            xgb_winner["nseg_n_trees"] = estim
            xgb_winner["nseg_tree_depth"] = depth
            xgb_winner["nseg_auc_test"] = mean_auc_fold_test
            xgb_winner["nseg_auc_val"] = mean_auc_fold
            xgb_winner["nseg_rf_object"] = classifier
            xgb_winner["nseg_acc_test"] = acc_test
            xgb_winner["nseg_acc_train"] = acc_train

            # Save winner model
            pickle_out = model_save_path + "xgb_nseg_winner.pkl"
            with open(pickle_out, 'wb') as file:
                pickle.dump(xgb_winner, file, protocol=pickle.HIGHEST_PROTOCOL)
            ...
            out_file = model_save_path + "xgb_nseg_best_auc.pdf"
            plt.plot([0, 1], [0, 1], 'k--')
            plt.xlim([-0.05, 1.0])
            plt.ylim([0.0, 1.05])
    ...
    print("logger_index: ", xgb_winner["logger_index"])
    print("nseg_n_trees", xgb_winner["nseg_n_trees"])
    print("nseg_tree_depth", xgb_winner["nseg_tree_depth"])
    print("nseg_auc_test", xgb_winner["nseg_auc_test"])
    print("nseg_auc_val", xgb_winner["nseg_auc_val"])
    print("nseg_acc_test", xgb_winner["nseg_acc_test"])
    print("nseg_acc_train", xgb_winner["nseg_acc_train"])
    ...
    save_dict = {"model_save_num": PARAMETERS['model_save_num'],
                                       ...
                                       "n_trees": xgb_winner["nseg_n_trees"],
                                       "tree_depth": xgb_winner["nseg_tree_depth"],
                                       "auc_test": xgb_winner["nseg_auc_test"],
                                       "auc_val": xgb_winner["nseg_auc_val"],
                                       "acc_train": xgb_winner["nseg_acc_train"],
                                       "acc_test": xgb_winner["nseg_acc_test"]}
